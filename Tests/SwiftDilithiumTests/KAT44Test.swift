...
final class KAT44Test: XCTestCase {
    
    override func setUpWithError() throws {
        let url1 = Bundle.module.url(forResource: "kat44KeyGen", withExtension: "rsp")!
        Util.makeKeyGenTests(&keyGenTests, try Data(contentsOf: url1))
        let url2 = Bundle.module.url(forResource: "kat44Sign", withExtension: "rsp")!
        Util.makeSignTests(&signTests, try Data(contentsOf: url2))
        let url3 = Bundle.module.url(forResource: "kat44Verify", withExtension: "rsp")!
        Util.makeVerifyTests(&verifyTests, try Data(contentsOf: url3))
        let url4 = Bundle.module.url(forResource: "kat44HashSign", withExtension: "rsp")!
        Util.makeHashSignTests(&hashSignTests, try Data(contentsOf: url4))
        let url5 = Bundle.module.url(forResource: "kat44HashVerify", withExtension: "rsp")!
    ...
    func testKeyGen() {
        Util.testKeyGen(.ML_DSA_44, keyGenTests)
    }
    
    func testSign() {
        Util.testSign(.ML_DSA_44, signTests)
    }

    func testVerify() {
        Util.testVerify(.ML_DSA_44, verifyTests)
    }

    func testHashSign() {
        Util.testHashSign(.ML_DSA_44, hashSignTests)
    }

    func testHashVerify() {
        Util.testHashVerify(.ML_DSA_44, hashVerifyTests)
    }
