...
final class KAT65Test: XCTestCase {
    
    override func setUpWithError() throws {
        let url1 = Bundle.module.url(forResource: "kat65KeyGen", withExtension: "rsp")!
        Util.makeKeyGenTests(&keyGenTests, try Data(contentsOf: url1))
        let url2 = Bundle.module.url(forResource: "kat65Sign", withExtension: "rsp")!
        Util.makeSignTests(&signTests, try Data(contentsOf: url2))
        let url3 = Bundle.module.url(forResource: "kat65Verify", withExtension: "rsp")!
        Util.makeVerifyTests(&verifyTests, try Data(contentsOf: url3))
        let url4 = Bundle.module.url(forResource: "kat65HashSign", withExtension: "rsp")!
        Util.makeHashSignTests(&hashSignTests, try Data(contentsOf: url4))
        let url5 = Bundle.module.url(forResource: "kat65HashVerify", withExtension: "rsp")!
    ...
    func testKeyGen() {
        Util.testKeyGen(.ML_DSA_65, keyGenTests)
    }

    func testSign() {
        Util.testSign(.ML_DSA_65, signTests)
    }

    func testVerify() {
        Util.testVerify(.ML_DSA_65, verifyTests)
    }

    func testHashSign() {
        Util.testHashSign(.ML_DSA_65, hashSignTests)
    }

    func testHashVerify() {
        Util.testHashVerify(.ML_DSA_65, hashVerifyTests)
    }
