...
struct Polynomial: Equatable {
    ...
    // ZETAS[i] = modPMQ(1753^bitreversal(i) % Q << 32)
    ...
    func NTT() -> Polynomial {
        ...
        while len >= 1 {
            ...
            while start < 256 {
                k += 1
                let zeta = Polynomial.ZETAS[k]
                for j in start ..< start + len {
                    let t = Polynomial.montgomeryReduce(zeta * w[j + len])
                    w[j + len] = w[j] - t
                    w[j] = w[j] + t
                }
                start += len << 1
    ...
    func INTT() -> Polynomial {
        ...
        while len < 256 {
            ...
            while start < 256 {
                ...
                for j in start ..< start + len {
                    ...
                    w[j] = t + w[j + len]
                    w[j + len] = t - w[j + len]
                    w[j + len] = Polynomial.montgomeryReduce(zeta * w[j + len])
                }
                start += len << 1
            }
            len <<= 1
        ...
        for i in 0 ..< 256 {
            w[i] = Polynomial.montgomeryReduce(w[i] * 41978) // 41978 = 2^64 / 256 % Q
        }
        return Polynomial(w)
    }

    static func montgomeryReduce(_ a: Int) -> Int {
        assert(-((1 << 31) * Dilithium.Q) <= a && a <= ((1 << 31) * Dilithium.Q))
        let t = ((((a << 32) >> 32) * 58728449) << 32) >> 32
        let r = (a - t * Dilithium.Q) >> 32
        assert(-Dilithium.Q < r && r < Dilithium.Q)
        return r
    }

    mutating func modPMQ() {
        for i in 0 ..< 256 {
            self.coef[i] = Dilithium.modPMQ(self.coef[i])
        }
    }
    ...
    func checkNorm(_ limit: Int) -> Bool {
        for i in 0 ..< 256 {
            if Dilithium.norm(self.coef[i]) >= limit {
                return true
            }
        }
        return false
    ...
    static func *(_ p1: Polynomial, _ p2: Polynomial) -> Polynomial {
        ...
        for i in 0 ..< 256 {
            x[i] = montgomeryReduce(p1.coef[i] * p2.coef[i])
        }
        return Polynomial(x)
    }
